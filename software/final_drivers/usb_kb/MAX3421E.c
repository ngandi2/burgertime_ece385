...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	wdata[0] = reg + 2;
	wdata[1] = val;
	int result = alt_avalon_spi_command(SPI0_BASE, 0, 2, wdata, 0, NULL, 0);
	if(result < 0) {
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	int result = alt_avalon_spi_command(SPI0_BASE, 0, nbytes + 1, wdata, 0, NULL, 0);
	if(result < 0) {
		printf("ERROR SPI TX RET = %x \n" , result);
	}
	return (data + nbytes);
...
BYTE MAXreg_rd(BYTE reg) {
	...
	alt_u8 rdata[1];
	wdata[0] = reg;
	int result = alt_avalon_spi_command(SPI0_BASE, 0, 1, wdata, 1, rdata, 0);
	if(result < 0) {
		printf("ERROR SPI TX RET = %x \n" , result);
	}
	return rdata[0];
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	alt_u8 wdata[1];
	wdata[0] = reg;
	int result = alt_avalon_spi_command(SPI0_BASE, 0, 1, wdata, nbytes, data, 0);
	if(result < 0) {
		printf("ERROR SPI TX RET = %x \n" , result);
	}
	return (data + nbytes);
